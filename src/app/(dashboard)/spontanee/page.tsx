  ...
  return (
    <div className="bg-black text-white font-sans selection:bg-blue-600 min-h-screen overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-[100] bg-black/80 backdrop-blur-xl border-b border-white/10">
        ...
        {menuOpen && (
          <div className="absolute top-20 left-0 w-full bg-black border-b border-white/10 p-8 flex flex-col gap-6 text-2xl font-black uppercase">
      ...
      <div className="p-8 max-w-6xl mx-auto space-y-20 py-20">
        <div className="grid md:grid-cols-3 gap-6">
          <PackCard title="Pack Starter" qty="500" price="20€" active={spontanee.packSelectionne === 'starter'} onClick={() => updateSpontanee({ packSelectionne: 'starter' })} />
          <PackCard title="Pack Pro" qty="1000" price="30€" active={spontanee.packSelectionne === 'pro'} onClick={() => updateSpontanee({ packSelectionne: 'pro' })} />
          <PackCard title="Pack Elite" qty="2000" price="50€" active={spontanee.packSelectionne === 'elite'} onClick={() => updateSpontanee({ packSelectionne: 'elite' })} />
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div className="space-y-6">
            <section className="bg-white/[0.03] border border-white/10 p-10 rounded-[3rem] space-y-8">
              <h2 className="text-2xl font-black flex items-center gap-4 uppercase tracking-tighter">
                <span className="bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center text-sm font-black italic">01</span> Documents (PDF)
              </h2>
              <div className="space-y-6">
                <Uploader label="Curriculum Vitae" file={cvFile} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFileUpload(e, 'cv')} />
                <Uploader label="Lettre de Motivation" file={letterFile} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleFileUpload(e, 'letter')} />
              </div>
            </section>
          </div>
          ...
          <div className="space-y-8">
            <section className="bg-white/[0.03] border border-white/10 p-10 rounded-[3rem] space-y-8 shadow-2xl">
              <h2 className="text-2xl font-black flex items-center gap-4 uppercase tracking-tighter">
                <span className="bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center text-sm font-black italic">02</span> Accès Techniques
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InputField label="Email Dédié" value={spontanee.emailDedicace} onChange={(v: string) => updateSpontanee({ emailDedicace: v })} placeholder="Email" />
                <InputField label="Pass Email" type="password" value={spontanee.passwordDedicace} onChange={(v: string) => updateSpontanee({ passwordDedicace: v })} placeholder="••••" />
                <InputField label="ID France Travail" value={spontanee.identifiantFT} onChange={(v: string) => updateSpontanee({ identifiantFT: v })} placeholder="ID" />
                <InputField label="Pass France Travail" type="password" value={spontanee.passwordFT} onChange={(v: string) => updateSpontanee({ passwordFT: v })} placeholder="••••" />
              </div>
            </section>

            <button onClick={() => { if(!cvFile || !letterFile || !spontanee.packSelectionne) return alert("Veuillez choisir un pack et joindre vos fichiers CV/Lettre !"); setShowPayment(true); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-6 rounded-2xl shadow-xl transition-all uppercase tracking-[0.2em] text-[10px]">
      ...
      {showPayment && (
        <div className="fixed inset-0 bg-black/95 z-[150] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-zinc-900 border border-white/10 p-10 rounded-[3rem] max-w-lg w-full space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(['Wave', 'PayPal', 'Wero', 'Virement'] as const).map(m => (
                  <button key={m} onClick={() => updatePaiement({ methodeUtilisee: m.toLowerCase() as any })} className={`p-4 rounded-xl border-2 text-[10px] font-black uppercase transition-all ${paiement.methodeUtilisee === m.toLowerCase() ? 'border-blue-600 bg-blue-600/10 text-white' : 'border-white/5 text-slate-500 hover:border-white/20'}`}>
                    {m}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4">
              <p className="text-xs font-black uppercase tracking-widest text-slate-500">Preuve (Reçu)</p>
            ...
            <button disabled={!proofFile || !paiement.methodeUtilisee || isSending} onClick={handleFinalSubmit} className="w-full bg-white text-black font-black py-5 rounded-2xl uppercase text-[10px] tracking-[0.2em] disabled:opacity-50 transition-all active:scale-95">
...
function InputField({ label, value, onChange, type = "text", placeholder }: any) {
  return (
    <div className="space-y-2">
      ...
      <input type={type} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} className="w-full bg-black border border-white/5 p-4 rounded-xl text-xs text-white focus:border-blue-500 outline-none transition-all shadow-inner placeholder:text-slate-700" />
    </div>
  );
}
...
function PackCard({ title, qty, price, active, onClick }: any) {
  return (
    <div onClick={onClick} className={`cursor-pointer p-10 rounded-[3rem] border-2 transition-all duration-500 ${active ? 'border-blue-600 bg-blue-600/10' : 'border-white/5 bg-white/[0.02] hover:border-white/20'}`}>
